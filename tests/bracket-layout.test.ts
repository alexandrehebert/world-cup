...
test('split rounds for condensed view using globally aligned rounds', () => {
  const rounds = [
    { id: 'roundOf32', matchIds: ['m73', 'm74', 'm75', 'm76', 'm77', 'm78', 'm79', 'm80', 'm81', 'm82', 'm83', 'm84', 'm85', 'm86', 'm87', 'm88'] },
    { id: 'roundOf16', matchIds: ['m89', 'm90', 'm91', 'm92', 'm93', 'm94', 'm95', 'm96'] },
    { id: 'quarterFinal', matchIds: ['m97', 'm98', 'm99', 'm100'] },
  ...
  const orderedMainRounds = alignSideRoundsByNextRound(rounds, matchesById, roundMatchIdsById)
  const preFinalRounds = orderedMainRounds.filter((round) => round.id !== 'final')
  const leftSplitBaseRounds = preFinalRounds.map((round) => ({
    ...round,
    matchIds: round.matchIds.slice(0, Math.ceil(round.matchIds.length / 2)),
  }))

  assert.deepEqual(leftSplitBaseRounds[1]?.matchIds, ['m89', 'm90', 'm93', 'm94'])
  assert.deepEqual(leftSplitBaseRounds[0]?.matchIds, ['m74', 'm77', 'm73', 'm75', 'm83', 'm84', 'm81', 'm82'])
})
