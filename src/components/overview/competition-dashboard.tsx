      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        <article className="border border-[var(--border)] bg-[var(--surface)] p-4 sm:p-5">
          ...
          {liveWidgetMatches.length === 0 ? (
            ...
            <div>
              {liveWidgetMatches.map((match, index) => {
              ...
              {liveWidgetMatches.length > 1 && (
                <div
                  className="relative overflow-hidden"
                  style={{
                    maskImage: `linear-gradient(to right, ${scrollState.canScrollLeft ? 'transparent 0px, black 40px' : 'black 0px'}, black calc(100% - ${scrollState.canScrollRight ? '40px' : '0px'}), ${scrollState.canScrollRight ? 'transparent 100%' : 'black 100%'})`,
                  }}
                >
                  <div ref={scrollContainerRef} className="flex gap-2 overflow-x-auto pb-1" style={{ scrollBehavior: 'smooth' }}>
                    {liveWidgetMatches.map((match, index) => {
                      ...
                      const homeScore = typeof match.home.score === 'number' ? match.home.score : null
                      const awayScore = typeof match.away.score === 'number' ? match.away.score : null
                      const stageLabel = match.stage === 'group' ? t.labels.stageGroup : match.stage === 'roundOf16' ? t.labels.stageRoundOf16 : match.stage === 'roundOf32' ? t.labels.stageRoundOf32 : match.stage === 'quarterFinal' ? t.labels.stageQuarterFinal : match.stage === 'semiFinal' ? t.labels.stageSemiFinal : match.stage === 'thirdPlace' ? t.labels.stageThirdPlace : t.labels.stageFinal
                        ...
                        >
                          ...
                          <div className="flex min-w-0 flex-1 items-center gap-2.5">
                            <div className="flex items-center gap-1.5">
                              {homeTeam ? <FlagAvatar team={homeTeam} className="h-5 w-5" /> : <span className="block h-5 w-5 rounded-full border border-dashed border-[var(--border)]" />}
                              <span className="truncate text-xs font-medium text-[var(--text)]">{homeTeamLabel}</span>
                            </div>
                            <span className="shrink-0 text-[10px] uppercase tracking-[0.16em] text-[var(--text-soft)]">{t.labels.vs}</span>
                            <div className="flex items-center gap-1.5">
                              <span className="truncate text-xs font-medium text-[var(--text)]">{awayTeamLabel}</span>
                              {awayTeam ? <FlagAvatar team={awayTeam} className="h-5 w-5" /> : <span className="block h-5 w-5 rounded-full border border-dashed border-[var(--border)]" />}
                            </div>
                          </div>
                          <div className="flex shrink-0 items-center gap-2">
                            {displayScore ? (
                              <span className="text-xs font-semibold text-[var(--text-strong)]">{homeScore ?? 0} - {awayScore ?? 0}</span>
                            ) : null}
                          </div>
                        </button>
                      )
